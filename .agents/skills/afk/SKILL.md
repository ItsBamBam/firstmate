...
- A message **without** the current operational prefix or a legacy bare marker, and **not** starting with `/afk` -> the captain is back.
  ...
  A single-owner task whose own worker has already concluded is the one exception: its leftover `blocked:` and `needs-decision:` rows are history, retired from the gate and from "waiting on you" through `bin/fm-classify-lib.sh`'s `status_task_concluded` (a secondmate's rows never retire that way).
