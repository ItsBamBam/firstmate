...
A refusal for uncommitted changes keeps the copy, its edits, and the record, but closes the endpoint of a worker that has already concluded (`fm-classify-lib.sh`'s `status_task_concluded`), so a finished worker never stays running because its cleanup is waiting on the captain; a still-live worker is never stopped by a refusal ([`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh)).
