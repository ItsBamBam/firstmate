...
That section prints every row's key, `default` for an unkeyed line, so its closing command is always typeable, and it retires the leftover rows of a single-owner task whose newest line is terminal (`fm-classify-lib.sh`'s `status_task_concluded`), because a worker that has concluded can no longer be answered and its landed or superseded work must not keep looking active while cleanup waits.
