...
The same header owns the refusal-time stop of a concluded worker: a landed-work refusal that retains the copy stops only the finished agent in place through `fm-control.sh`'s `exit`, keeping the endpoint relaunchable, and only once no-mistakes proves no run of the task's own still needs it; [`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh) pins that stop.
