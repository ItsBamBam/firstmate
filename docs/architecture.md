...
A landed-work refusal (uncommitted changes, unmerged, or unpushed work) keeps the copy, its work, and the record, but for a worker that has already concluded (`fm-classify-lib.sh`'s `status_task_concluded`) it closes the recorded endpoint under the same locks a completed teardown uses and reports only the endpoint state it observes afterwards, so a finished worker never stays running because its cleanup is waiting on the captain and is never reported stopped while it still runs; the refusal never aborts a parked no-mistakes run (that worker is retained to answer its gate), never reaps processes rooted in the retained copy, and never stops a still-live worker ([`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh)).
