...
A refusal for uncommitted changes keeps the copy, its edits, and the record, but closes the endpoint of a worker that has already concluded (`fm-classify-lib.sh`'s `status_task_concluded`), through the same locked close a completed teardown uses and reports only the endpoint state it observes afterwards, so a finished worker never stays running because its cleanup is waiting on the captain and is never reported stopped while it still runs; a still-live worker is never stopped by a refusal ([`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh)).
