...
A landed-work refusal (uncommitted changes, unmerged, or unpushed work) keeps the copy, its work, the record, and the endpoint, but for a worker that has already concluded (`fm-classify-lib.sh`'s `status_task_concluded`) it stops the agent in place through the control plane's own `exit` mechanics (`fm-control.sh`), so the endpoint stays relaunchable for the captain's keep answer while a finished worker never stays running because its cleanup is waiting on the captain; the refusal never closes the endpoint, never aborts a no-mistakes run, retains the worker whenever no-mistakes cannot prove that no run of the task's own still needs it (the CLI's positive no-run answer for the branch is proof; a failed or timed-out query is not, and a local-only task never asks), never reaps processes rooted in the retained copy, never stops a still-live worker, and claims a stop only from the control plane's verified postcondition, saying why otherwise ([`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh)).
