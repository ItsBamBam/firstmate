...
A landed-work refusal (uncommitted changes, unmerged, or unpushed work) keeps the copy, its work, and the record, but for a worker that has already concluded (`fm-classify-lib.sh`'s `status_task_concluded`) it runs the same pre-teardown sequence a completed teardown does - conclude the parked no-mistakes run, reap processes rooted in the worktree or task temp, then close the recorded endpoint under the same locks - and reports only the endpoint state it observes afterwards, so a finished worker never stays running because its cleanup is waiting on the captain and is never reported stopped while it still runs; a process the reap cannot prove is the task's own is reported rather than killed, and a still-live worker is never stopped by a refusal ([`tests/fm-teardown.test.sh`](../tests/fm-teardown.test.sh)).
