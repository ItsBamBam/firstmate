...
origin_open_decisions() {  # <origin-id>
  local origin=$1 meta="$STATE/$1.meta" status_file="$STATE/$1.status" open
  open=$(status_open_decisions "$status_file")
  [ -n "$open" ] || return 0
  # An origin whose record is gone keeps its raw fold: only a recorded
  # single-owner task can be read as concluded (bin/fm-classify-lib.sh's
  # status_task_concluded).
  [ -f "$meta" ] || { printf '%s' "$open"; return 0; }
  if status_task_concluded "$status_file" "$meta"; then
    return 0
  fi
  printf '%s' "$open"
}
