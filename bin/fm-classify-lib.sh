...
# bin/fm-afk-return.sh's blocker gate and return brief, bin/fm-captain-hold.sh's
# origin gate, and bin/fm-teardown.sh's finished-worker stop at every
# landed-work refusal that retains the copy (bin/fm-fleet-snapshot.sh clears
# on the reconciled crew state instead, which already covers this terminal
# read).
