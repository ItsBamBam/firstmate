...
# missed. Rows of a task whose own worker has concluded are retired (see the
# loop below). Informational `note:` lines and pending-reply resolutions are
# not decisions; print_unread_status_section owns their one-shot surface. Runs on
